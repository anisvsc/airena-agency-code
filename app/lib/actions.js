...
export const createStream = async (formData) => {
  ...
  try {
    ...
    stream = (await livepeer.stream.create({ name: formData.get("title") }))
      .stream;
    let thumbnailUrl = "";
    if (thumbnailFile && typeof thumbnailFile.name === "string") {
      ...
      const storageRef = ref(
        storage,
        `thumbnails/${stream.id}/${thumbnailFile.name}`,
      );
    ...
    await setDoc(
      doc(db, "streams", influencerId),
      {
        [stream.id]: streamData, // Use stream ID as the key for the stream data
      },
      { merge: true },
    ); // Use merge to preserve other streams under this influencer
  } catch (error) {
...
export const updateStreamTitle = async (streamId, newTitle) => {
  try {
    const updatedStream = await livepeer.stream.update(
      { name: newTitle },
      streamId,
    );
...
export const getLiveViews = async (playbackId) => {
  try {
    const views = (await livepeer.metrics.getRealtimeViewership(playbackId))
      ?.data[0]?.viewCount;
    return views;
  } catch (error) {
...
export const endStream = async (streamId, influencerId) => {
  try {
    ...
    const startTime = streamData.streamStartTime
      ? new Date(streamData.streamStartTime)
      : null;
    ...
    await setDoc(
      doc(db, "streams", influencerId),
      {
        [streamId]: {
          ...
          streamStartTime:
            isActive && !streamData.streamStartTime
...
export const updateStreamProducts = async (
  streamId,
  influencerId,
  products,
) => {
