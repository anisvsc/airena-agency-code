...
export default function ProductSelection({ open, onClose, onProductsSelect }) {
  ...
  const fetchProducts = async (vendor = "") => {
    ...
    try {
      const response = await fetch(
        `/api/vendor/fetch_products?vendor=${encodeURIComponent(searchQuery)}`,
      );
      if (!response.ok) {
  ...
  const handleProductSelect = (product) => {
    if (selectedProducts.find((p) => p.node.id === product.node.id)) {
      setSelectedProducts(
        selectedProducts.filter((p) => p.node.id !== product.node.id),
      );
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>Select Products for Advertisement (Max 2)</DialogTitle>
      <DialogContent>
              ...
              <Grid item xs={12} sm={6} md={4} key={product.node.id}>
                <Card
                  className={`cursor-pointer transition-all ${
                    selectedProducts?.find((p) => p.node.id === product.node.id)
